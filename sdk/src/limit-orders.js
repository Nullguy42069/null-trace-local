...
export class LimitOrders {
  ...
  async createTakeProfit(params) {
    ...
    // Calculate trigger from gain % if provided
  ...
  async createStopLoss(params) {
    ...
    // Calculate trigger from loss % if provided
    ...
    return this._createOrder({
      ...
      amount,
      triggerPrice: finalTriggerPrice,
      slippage: params.slippage || 0.15, // Higher default slippage for SL
  ...
  async createBracketOrder(params) {
    ...
    // Link orders (cancel other when one executes)
    ...
    for (const order of pending) {
      try {
        ...
        if (result.success) {
          ...
          // Cancel linked order (bracket OCO behavior)
